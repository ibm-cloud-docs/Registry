...
Create a namespace to store your container images in {{site.data.keyword.registrylong_notm}}. Namespaces are created in a [resource group](#x2161955){: term}. For more information, see [Planning namespaces](/docs/Registry?topic=Registry-registry_setup_cli_namespace#registry_setup_cli_namespace_plan).
...
1. To log in to {{site.data.keyword.cloud_notm}} and target the `us-south` region, run the following command.
    ...
    If you have a federated ID, use `ibmcloud login -r us-south --sso` to log in. Enter your username and use the provided URL in your command-line interface (CLI) output to retrieve your one-time passcode. If you have a federated ID, the login fails without the `--sso` and succeeds with the `--sso` option.
    {: requirement}
