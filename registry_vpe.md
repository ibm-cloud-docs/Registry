...
- Ensure that a Virtual Private Cloud is created. For more information, see [Getting started with Virtual Private Cloud](/docs/vpc?topic=vpc-getting-started).
- Make a plan for your virtual private endpoints. For more information, see [Planning for virtual private endpoint gateways](/docs/vpc?topic=vpc-vpe-planning-considerations).
- Ensure that the correct access controls are set for your VPE. For more information, see [Configuring ACLs and security groups for use with endpoint gateways](/docs/vpc?topic=vpc-configure-acls-sgs-endpoint-gateways).
- Ensure that you understand the limitations of having a VPE. For more information, see [Planning for virtual private endpoint gateways](/docs/vpc?topic=vpc-vpe-planning-considerations).
- Ensure that you understand how to view details about a VPE. For more information, see [Viewing details of an endpoint gateway](/docs/vpc?topic=vpc-vpe-viewing-details-of-an-endpoint-gateway).
...
You can create VPE gateways in these locations:  `au-syd`, `br-sao`, `ca-mon`, `ca-tor`, `eu-de`, `eu-es`, `eu-gb`, `in-che`, `in-mum`, `jp-osa`, `jp-tok`, `us-south`, and `us-east` (global registry).
...
| `in-mum` | Not applicable | `crn:v1:bluemix:public:container-registry:in-mum:::endpoint:in2.icr.io` |
