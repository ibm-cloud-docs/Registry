...
通过 Container Image Security Enforcement (Beta)，可以在将容器映像部署到 {{site.data.keyword.containerlong}} 中的集群之前，对这些映像进行验证。您可以控制从何处部署映像，强制实施漏洞顾问程序策略，以及确保[内容信任](registry_trusted_content.html)正确应用于映像。如果映像不符合策略需求，那么不会将 pod 部署到集群，也不会更新 pod。
{:shortdesc}
...
可以在策略中启用 `trust` 选项来强制实施内容信任。如果未指定任何 `signerSecrets`，那么允许部署由任何人签名的映像。如果指定 `signerSecrets`，那么最近签名的映像版本必须已由您指定的所有签署者签名。Container Image Security Enforcement 会验证提供的公用密钥是否属于签署者。有关内容信任的更多信息，请参阅[对映像签名以实现可信内容](registry_trusted_content.html)。
