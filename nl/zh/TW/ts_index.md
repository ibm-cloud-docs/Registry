...
使用討論區提問時，請標記您的問題，以便 {{site.data.keyword.registrylong_notm}} 開發團隊能看到它。

-   如果您在使用 {{site.data.keyword.registrylong_notm}} 開發或部署應用程式時有技術方面的問題，請將問題張貼在 [Stack Overflow](http://stackoverflow.com/search?q=+ibm-bluemix)，並使用 `ibm-bluemix` 和 `container-registry` 來標記問題。
-   若為服務及開始使用指示的相關問題，請使用 [IBM developerWorks dW Answers](https://developer.ibm.com/answers/topics/container-registry/?smartspace=bluemix) 討論區。請包含 `bluemix` 和 `container-registry` 標籤。

如需使用討論區的詳細資料，請參閱[使用支援中心](../../get-support/howtogetsupport.html#using-avatar)。如需開立 {{site.data.keyword.IBM_notm}} 支援問題單的相關資訊，或支援層次與問題單嚴重性的相關資訊，請參閱[開立支援問題單](../../get-support/howtogetsupport.html#open-ticket)。
...
-   您的 {{site.data.keyword.Bluemix_notm}} 登入認證已到期。
...
-   最近已刪除名稱空間，而您要重複使用其名稱。如果已刪除的名稱空間包含許多資源，則 {{site.data.keyword.registrylong_notm}} 可能尚未完整處理這項刪除。
-   您已在名稱空間值中使用無效字元。
...
-   您的 {{site.data.keyword.Bluemix_notm}} 存取記號可能已到期。
...
{: tsCauses}
依預設，會套用 `latest` 標籤，以在未指定標籤值的情況下執行 Docker 指令時參照映像檔。`latest` 標籤會套用至在未明確設定標籤值的情況下執行的最新 `docker build` 或 `docker tag` 指令。因此，可能可以不正常地執行 `docker` 指令，或明確地設定部分映像檔的標籤，以及設定 `latest` 標籤來參照非最新的建置。
...
{: tsSymptoms}
當您嘗試匯入您在其他 IBM 產品（例如 {{site.data.keyword.Bluemix_notm}} Private）中所使用的內容時，您無法將來自 [IBM Passport Advantage ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://www-01.ibm.com/software/passportadvantage/index.html) 的映像檔及其他授權軟體儲存在登錄中。

{: tsCauses}
套裝軟體（例如來自 IBM Passport Advantage 的映像檔和 Helm 圖表）必須使用 `bx cr ppa-archive-load` 指令匯入到登錄。

{: tsResolve}
開始之前：
* 執行 `bx login [--sso]`，以登入 {{site.data.keyword.Bluemix_notm}}。
* 執行 `bx cr login`，以登入 {{site.data.keyword.registrylong_notm}}。
* [將 `kubectl` CLI 的目標](../../containers/cs_cli_install.html#cs_cli_configure)設為叢集。
* 如果您尚未設定叢集中的 Helm，請[立即在叢集中設定 Helm](../../containers/cs_integrations.html#helm)。
* 如果您要在組織中共用圖表，可以安裝 [Chart Museum 開放程式碼專案 ![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://github.com/kubernetes/charts/tree/master/stable/chartmuseum)。

### 匯入 IBM Passport Advantage 產品以便用於 {{site.data.keyword.Bluemix_notm}}

1.  從 [IBM Passport Advantage![外部鏈結圖示](../../icons/launch-glyph.svg "外部鏈結圖示")](https://www-01.ibm.com/software/passportadvantage/index.html) 取得您要匯入的壓縮檔。
...
3.  匯入壓縮保存檔。指定壓縮檔的路徑，以及您要將映像檔推送至其中的登錄名稱空間。
    ...
    此指令會展開壓縮檔，將任何包含的映像檔載入至本端 Docker 用戶端，然後將映像檔推送至登錄中的名稱空間。
    
    如果您要將 Helm 圖表從 IBM Passport Advantage 保存檔上傳至 Chart Museum，請在指令中包括下列選項：`bx cr ppa-archive-load --archive</path/to/archive.tgz> --namespace <namespace> --chartmuseum-uri <URI> --chartmuseum-user <user_name> --chartmuseum-password <password>`
    {: tip}
...
4.  如果壓縮檔包含 Helm 圖表，這些圖表會放在您現行工作目錄中建立的保存目錄，稱為 `ppa-import`。開啟目錄以取得 Helm 圖表 `<helm_chart>` 的名稱，然後檢查其值。
    ...
    如果您已在前一個步驟中將圖表上傳至 Chart Museum，可以使用 `helm inspect` 在 Chart Museum 中檢查該圖表。
    {: tip}

5.  根據 `helm inspect values` 指令輸出的值，配置 Helm 圖表 `<helm_chart>`。

6.  使用 `helm install` 指令部署 Helm 圖表 `<helm_chart>`。您可以使用 `--set` 選項，視需要置換圖表中的值。
...
1.  記下您要用來連接 {{site.data.keyword.registrylong_notm}} 之機器的公用 IP 位址。如果使用 Kubernetes，請使用您的工作者節點的公用 IP 位址。請執行 `bx cs workers <cluster_name_or_id>`，以擷取工作者節點的公用 IP 位址，其中 *&lt;cluster_name_or_id&gt;* 是您的叢集名稱或 ID。
...
{: tsResolve}
您用來回復遺失或受影響之金鑰的選項，取決於金鑰類型 - 儲存庫或主要：

*  對於[儲存庫金鑰](#trustedcontent_lostrepokey)，您可以為儲存庫產生一組新的簽署金鑰。
*  對於[主要金鑰](#trustedcontent_lostrootkey)，您可以要求刪除儲存庫，並建立一個新的儲存庫。
...
**附註**：您唯一可以輪替的簽署角色是 `targets`，這是儲存庫管理員。如果其他角色受到影響，請為那些角色產生新的金鑰、移除舊的金鑰，並將新的金鑰新增為簽章者。

開始之前，請擷取您第一次[推送已簽署的映像檔](registry_trusted_content.html#trustedcontent_push)時所建立的主要金鑰通行詞組。

1.  安裝 [Notary 專案](https://github.com/theupdateframework/notary#getting-started-with-the-notary-cli)的 CLI 版本。
...
5.	替換您的金鑰，以便使用那些金鑰所簽署的內容不再受到信任。將 _&lt;URL&gt;_ 取代為您在步驟 2 中記下的匯出指令 URL，並將 _&lt;image&gt;_ 取代為儲存庫金鑰受到影響的映像檔。
...
6.	如果出現提示，請輸入主要金鑰通行詞組。然後，在提示時輸入新的儲存庫金鑰的新主要金鑰通行詞組。
...
您可以[刪除具有使用受影響主要金鑰之儲存庫的名稱空間](registry_setup_cli_namespace.html#registry_remove)，這樣會刪除您的映像檔和信任資料。

如果名稱空間包含的儲存庫具有不受影響的主要金鑰（例如正式作業映像檔的名稱空間），則建議您只刪除與受影響之主要金鑰相關聯的信任資料。開立支援問題單。

1.  [與 {{site.data.keyword.Bluemix_notm}} 支援中心聯絡](../../get-support/howtogetsupport.html)。請包含問題的簡要說明、帳戶 ID，以及包含具有受影響主要金鑰之映像檔儲存庫的名稱空間清單。

2.  在 {{site.data.keyword.Bluemix_notm}} 處理該問題後，刪除本端機器上的 Docker Content Trust 儲存庫。

    * Linux 及 Mac 目錄：`~/.docker/trust/private` 及 `~/.docker/trust/tuf`

    * Windows 目錄：`%HOMEPATH%\.docker\trust\private` 及 `%HOMEPATH%\.docker\trust\tuf`

    **附註**：因為主要金鑰受到影響，所以此步驟會刪除所有簽署金鑰，包括其他信任伺服器的簽署金鑰。

3.  如果您在 {{site.data.keyword.containershort_notm}} 叢集中使用 [{{site.data.keyword.Bluemix_notm}} Image Enforcement](registry_security_enforce.html)，請重新啟動每一個映像檔強制執行 Pod。若要觸發 Kubernetes 以自動執行 Pod 的漸進式重新啟動，您可以變更 Pod 上的部分 meta 資料。例如，[將您的 Kubernetes CLI 目標設為叢集](../../containers/cs_cli_install.html#cs_cli_configure)，並修改部署。
    ```
...
4.  產生受信任內容儲存庫。
    ...
    *  如果您不想變更先前的受信任內容，請新增簽章至登錄中的最新映像檔。
...
## Container Image Security Enforcement 的安裝失敗，發生 `helm install ibm-incubator/ibmcloud-image-enforcement --name cise Error: jobs.batch "create-crds" already exists`
