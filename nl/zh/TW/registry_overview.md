...
## {{site.data.keyword.registrylong_notm}} 地區
{: #registry_regions}
...
-   EU 中部：`registry.eu-de.bluemix.net`
-   英國南部：`registry.eu-gb.bluemix.net`
-   美國南部：`registry.ng.bluemix.net`

所有登錄構件的範圍設定為您目前使用中的特定地區登錄。例如，名稱空間、映像檔、記號、配額設定及方案設定必須全都針對每個地區登錄分開管理。

如果您想要使用非本端地區的地區，可以執行 `bx target` 指令搭配 `-r` 旗標設定目標地區，其中 _&lt;region&gt;_ 是地區的名稱（`us-south`、`eu-de`、`eu-gb` 或 `au-syd`）。
