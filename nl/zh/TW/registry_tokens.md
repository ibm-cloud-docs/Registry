...
您嘗試在 Kubernetes 部署中使用您的登錄映像檔嗎？請參閱[存取其他 Kubernetes 名稱空間、{{site.data.keyword.Bluemix_notm}} 地區和帳戶中的映像檔](/docs/containers/cs_images.html#other)。
{: tip}
...
**附註：**會自動從 {{site.data.keyword.Bluemix_notm}} 帳戶移除到期的 {{site.data.keyword.registrylong_notm}} 記號，不需要手動予以移除。
