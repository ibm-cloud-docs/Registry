...
|`SecurityStatus`|구조체|이미지에 대한 취약성 상태를 표시합니다. 다음 값을 필터링하고 형식화할 수 있습니다. Status `string`, IssueCount `int` 및 ExemptionCount `int`. 가능한 상태는 [CLI를 사용하여 취약성 보고서 검토](/docs/services/Registry?topic=va-va_index#va_registry_cli)에 설명되어 있습니다.|
