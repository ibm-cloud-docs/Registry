...
IBM Container Image Security Enforcement (ベータ) を使用すると、{{site.data.keyword.containerlong}} のクラスターにデプロイする前にコンテナー・イメージを検査できます。イメージのデプロイ元を制御し、脆弱性アドバイザーのポリシーを適用して、[コンテント・トラスト](registry_trusted_content.html)をイメージに適切に適用することができます。ポッドがポリシーの要件を満たさない場合、クラスターは変更されません。
{:shortdesc}

IBM Container Image Security Enforcement は、イメージのコンテント・トラストと脆弱性に関する情報を {{site.data.keyword.registrylong}} から取得します。他のレジストリーのイメージについては、ブロックするか許可するかをポリシーで選択することはできますが、脆弱性やトラストの制約を使用することはできません。
...
1.  IBM のチャート・リポジトリーを Helm に追加します。
...
1.  IBM Container Image Security Enforcement の Helm チャートを `ibm-system` クラスター名前空間にインストールします。`<cise>` などの名前を指定します。
...
IBM Container Image Security Enforcement は、セキュリティー・ポリシーを作成するための開始点として使用できるように、いくつかのポリシーをデフォルトでインストールします。{:shortdesc}
...
デフォルトでは、`kube-system` 名前空間に対して名前空間規模のポリシーがインストールされます。このポリシーは、すべてのコンテナー・レジストリーのすべてのイメージを `kube-system` にデプロイすることを制約なしで許可します。また、使用されたリポジトリーにクラスターを構成することも許可します。{:shortdesc}
...
デフォルトでは、`ibm-system` 名前空間に対して名前空間規模のポリシーがインストールされます。このポリシーは、すべてのコンテナー・レジストリーのすべてのイメージを `ibm-system` にデプロイすることを制約なしで許可します。また、使用されたリポジトリーに、クラスターの構成、Image Security Enforcement のインストールまたはアップグレードを行うことも許可します。
{:shortdesc}
...
クラスター・レベルまたは Kubernetes 名前空間レベルでイメージを許可するために IBM Container Image Security Enforcement で使用されるポリシーは、ユーザーが変更できます。ポリシーには、さまざまなイメージに対するさまざまな制約ルールを指定できます。
{:shortdesc}
...
Kubernetes クラスターで役割ベースのアクセス制御 (RBAC) を有効にしている場合は、役割を作成して、クラスターのセキュリティー・ポリシーを管理できるユーザーを制御できます。クラスターに RBAC ルールを適用する方法について詳しくは、[IBM Cloud Container Service の資料](../../containers/cs_users.html#rbac)を参照してください。
{:shortdesc}
...
**注**: `cluster-admin` 役割を持つユーザーおよび ServiceAccount は、すべてのリソースにアクセスできます。cluster-admin 役割は、編集を行わなくても、セキュリティー・ポリシーを管理する権限を付与します。`cluster-admin` 役割を持つユーザーの制御を必ず行い、セキュリティー・ポリシーの変更を許可するユーザーだけにアクセス権限を付与するようにしてください。

## セキュリティーを強化したコンテナー・イメージのデプロイ
{: #deploy_containers}

ポリシーを適用した後は、通常どおりにコンテントをクラスターにデプロイできます。ポリシーは、Kubernetes クラスターによって自動的に適用されます。デプロイメントがポリシーに一致し、そのポリシーで許可されると、そのデプロイメントはクラスターに承認され、適用されます。
{:shortdesc}

Container Image Security Enforcement がデプロイメントを拒否した場合、デプロイメントは作成されますが、そのデプロイメントによって作成された ReplicaSet はスケールアップできず、ポッドは作成されません。`kubectl describe deployment <deployment-name>` を実行して ReplicaSet を見つけ、`kubectl describe rs <replicaset-name>` を実行してデプロイメントが拒否された理由を確認できます。
