...
<dl>
  <dt>レジストリー</dt>
  <dd>レジストリーは、Docker イメージを保管するためのインフラストラクチャーを提供するサービスで、レジストリー・ホスト URL とオプションのポートを使用してアクセスすることができます。 だれでもアクセスできるレジストリー (パブリック・レジストリー) を使用することも、小規模なユーザーのグループだけにアクセスを限定したレジストリー (専用レジストリー) をセットアップすることもできます。 {{site.data.keyword.registrylong_notm}} では、マルチテナントで可用性の高い、IBM によってホストされ管理されている専用イメージ・レジストリーを利用できます。 ユーザーは独自のイメージ名前空間をセットアップして専用レジストリーを使用し、名前空間への Docker イメージのプッシュを開始できます。</dd>
</dl>
...
<dl>
  <dt>イメージ</dt>
  <dd>Docker イメージは、Dockerfile の指示に基づいてビルドされるもので、コンテナーの基礎となります。 Docker イメージをビルドしたら、それを使用してコンテナーを作成し、アプリとその従属関係をデプロイすることができます。 イメージはレジストリーに保管されます。 {{site.data.keyword.Bluemix_notm}} アカウントへのアクセス権を持つユーザーは、対象のイメージにアクセスできます。</dd>
</dl>
...
**注**: アカウントに名前空間が既に設定されているかどうかが不明な場合は、`bx cr namespace-list` コマンドを実行して、既存の名前空間に関する情報を取得してください。 [単一コンテナーおよびスケーラブル・コンテナー・グループ](../../containers/cs_classic.html)を使用している、既存の {{site.data.keyword.containerlong_notm}} のお客様の場合、名前空間が既に設定されています。 追加の名前空間を作成できますが、複数の名前空間のために `cf ic namespace set` を実行することはできません。
...
別の地域をターゲットにした後、`bx cr login` を実行して、レジストリーに再度ログインします。
...
`bx cr region-set` コマンドを実行して、グローバル・レジストリーをターゲットにすることができます。
...
`bx cr region-set` コマンドについて詳しくは、[{{site.data.keyword.registrylong_notm}} CLI](registry_cli.html#bx_cr_region_set) を参照してください。

グローバル・レジストリーをターゲットにした後に、`bx cr login` コマンドを実行してローカル Docker デーモンをグローバル・レジストリーにログインさせ、{{site.data.keyword.IBM_notm}} 提供のパブリック・イメージをプルできるようにします。
