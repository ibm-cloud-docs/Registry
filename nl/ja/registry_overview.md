...
アカウントに名前空間が既に設定されているかどうかが不明な場合は、`ibmcloud cr namespace-list` コマンドを実行して、既存の名前空間に関する情報を取得してください。 [単一コンテナーおよびスケーラブル・コンテナー・グループ](../../containers/cs_classic.html)を使用している、既存の {{site.data.keyword.containerlong_notm}} のお客様の場合、名前空間が既に設定されています。 追加の名前空間を作成できますが、複数の名前空間のために `cf ic namespace set` を実行することはできません。
{:tip}
