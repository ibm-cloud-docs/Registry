...
Vous pouvez vérifier les versions signées du référentiel ou de l'étiquette d'une image signée, y compris les informations sur l'ID clé et le signataire.
{:shortdesc}
...
2.  Retirez toutes les métadonnées sécurisées pour le référentiel d'images. Entrez votre phrase passe de clé de référentiel lorsque vous y êtes invité. **Facultatif** : Spécifiez une étiquette pour révoquer les métadonnées sécurisées uniquement pour cette version de l'image.
