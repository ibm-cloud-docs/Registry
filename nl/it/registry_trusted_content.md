...
Un nome immagine è costituito da un repository e una tag. Quando si utilizzano i contenuti attendibili, ciascun repository utilizza una chiave di firma univoca. Ogni tag all'interno di un repository utilizza la chiave che appartiene al repository. Se ci sono più team che pubblicano dei contenuti, ciascuno nel proprio repository all'interno di tuoi spazi dei nomi di {{site.data.keyword.registrylong_notm}}, ogni team può utilizzare le proprie chiavi per firmare i propri contenuti, in modo che tu possa verificare che ogni immagine sia prodotta dal team appropriato.
...
2.  Rimuovi tutti i metadati attendibili per il repository di immagini. Immetti la passphrase della chiave di repository quando richiesto. **Facoltativo**: specifica una tag per revocare i metadati attendibili solo per quella versione dell'immagine.
...
3.  Verifica che l'attendibilità sia stata revocata nell'elenco di contenuti attendibili. **Facoltativo**: includi la tag se vuoi verificare il contenuto revocato per un'immagine con tag.
...
Quando esegui per la prima volta il push di un'immagine firmata in un nuovo repository, Docker Content Trust crea due chiavi di firma, chiave root e chiave di repository, e le memorizza nella tua macchina locale:
