...
# Tutorial: Granting access to {{site.data.keyword.registrylong_notm}} resources
{: #iam_access}
...
- Ensure that you have the most recent version of the `container-registry` CLI plug-in for the {{site.data.keyword.cloud_notm}} CLI, see [Updating the `container-registry` CLI plug-in](https://console.bluemix.net/docs/services/Registry/registry_setup_cli_namespace.html#registry_cli_update).
...
3. Create policies to grant User B the ability to interact with the namespaces by running the following command:
    ...
    3. Create a policy that grants the Reader role on `namespace_b` to User B by running the following command, where _`<Region>`_ is the short name for your [region](/docs/services/Registry/registry_overview.html#registry_regions), for example `us-south`:
