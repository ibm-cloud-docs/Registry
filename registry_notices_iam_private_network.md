...
Also, as part of this change, the {{site.data.keyword.registrylong}} service private IP addressees are changing which may require updates to your firewall configuration.

This change was originally due to take place on 23 May 2022 but had to be delayed. You should not remove the {{site.data.keyword.registrylong}} private IP addresses from your IAM restricted IP list until a later announcement advises you to do so.
...
:   You must update your IAM restricted IP address list to include any IP addresses or subnets of hosts in your account that make requests to {{site.data.keyword.registryshort}}. You should keep the current {{site.data.keyword.registryshort}} private IP addresses in your restricted IP list until a later announcement indicates it is safe to do so. For information about how to update an restricted IP address list, see [Allowing specific IP addresses](/docs/account?topic=account-ips) in the {{site.data.keyword.iamshort}} (IAM) documentation.

If you have firewalls configured with the {{site.data.keyword.registryshort}} private IP addresses.
