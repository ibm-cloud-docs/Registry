...
A security token allows everyone in possession of the token to access secured information. Tokens are used in a similar way to API keys. By creating a token for your account, you can grant access to all of your namespaces that you set up in a region for users outside your account. Every user or app in possession of this token can push and pull images to and from your namespaces without installing the container-registry plug-in.

When you create a token for your account, you can decide whether that token authorizes read-only (pull) or write access (push and pull) to the registry. You can also specify whether a token is permanent or if it expires after 24 hours. You can create and use multiple tokens to control different types of access.


## Creating an {{site.data.keyword.registrylong_notm}} token for your account
...
## Removing an {{site.data.keyword.registrylong_notm}} token from an account
{: #registry_tokens_remove}

Remove a token when you do not need it anymore.
{:shortdesc}

**Note:** Expired tokens are removed automatically from your account and do not need to be removed manually.
