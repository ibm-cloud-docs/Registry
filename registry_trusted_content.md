...
A word of caution: the default configuration when signing the using `skopeo` command is that signatures remain on the local file system.  This includes when images are signed inside of pipelines on OpenShift clusters.  If you are pushing signed images the IBM Container Registry, yet your deployments to clusters that use Image Security Enforcement are failing with the message `policy denied the request: A signature was required, but no signature exists`, then the default policy is likely saving your image signatures locally instead of pushing the signature to the API server.  This configuration is saved in `/etc/containers/registries.d/default.yaml`.  To have your signatures pushed to the IBM Cloud container registy along with your image you will need to modify this configuration, but the most straightforward solution is to remove it altogether using `rm /etc/containers/registries.d/default.yaml`. 
